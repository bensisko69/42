...
/*   Updated: 2014/02/15 18:05:33 by lrenoud-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_setenv(t_list **list, char *line)
{
	...
	{
		cmp1 = ft_strjoin(cmp[1], "=");
		if (search(*list, cmp1) == 0)
		{
...
void	exchange_content(t_list *list, char *cmp2, char *cmp3)
{
	while (list != NULL)
	{
		if (ft_strnequ(list->str, cmp2, ft_strlen_char(list->str, '=')) == 1)
		{
			free(list->str);
			list->str = cmp3;
			break ;
